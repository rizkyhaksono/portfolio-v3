...
export default function CardBlog({
  ...
  date,
  external = false,
}: Readonly<CardBlogProps>) {
  return (
    <article className="group border-t border-border last:border-b">
      <Link href={href} target={external ? "_blank" : undefined} rel={external ? "noreferrer" : undefined} className="grid grid-cols-[1fr_auto] items-center gap-5 py-6 sm:py-8">
        <div className="min-w-0">
          <div className="mb-3 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
            <span className="border border-border bg-secondary px-2 py-1 text-foreground">{source}</span>
            <span>Article</span>
            {date ? <><span aria-hidden="true">·</span><time>{date}</time></> : null}
          </div>
          ...
          <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium">
            {external ? `Read on ${source}` : "Read article"} <ArrowUpRight className="h-3.5 w-3.5" />
          </span>
        </div>
        {image && (
